  ...
  <v-container class="pa-0">
    <v-card class="elevation-2 rounded-lg" outlined>
      <v-card-title class="pb-2">
        <v-icon color="indigo" class="mr-2">mdi-star</v-icon>
        <span class="text-h6 font-weight-medium">팬버튼 메시지 설정</span>
      </v-card-title>
      <v-card-text class="pt-2">
        <v-textarea
            ...
            v-model="liveFollow"
            label="팬버튼 시 표시할 메시지"
            placeholder="시청자가 팬버튼을 눌렀을 때 표시될 메시지를 입력하세요..."
            counter
            filled
            rows="4"
            auto-grow
...
export default class CmdFollow extends Mixins(GlobalMixins) {
	public liveFollow: string = '';

	public cfgPath: string = this.$path('userData', 'cmd.cfg');
	public cfg: CfgLite = new CfgLite(this.cfgPath);

	public async mounted() {
		this.liveFollow = this.cfg.get('live_follow') || '';
		this.$evt.$on('cmd:save', () => {
			this.cfg.set('live_follow', this.liveFollow);
			this.cfg.save();
		});
	}
